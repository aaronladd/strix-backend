...
sub fileRebuild{
	...
	if ($contact == 1){
		system "/home/nagios/scripting/strix-backend/file_scripts/rebuild_contact.pl $accountId 2>&1 >> /var/log/nagios/contact_rebuilding.log &";
	}
	
	if ($cgroup == 1){
		system "/home/nagios/scripting/strix-backend/file_scripts/rebuild_groups.pl $accountId 2>&1 >> /var/log/nagios/group_rebuilding.log &";
	}
	
	if ($host == 1){
		system "/home/nagios/scripting/strix-backend/file_scripts/rebuild_host.pl $accountId 2>&1 >> /var/log/nagios/host_rebuilding.log &";
	}
